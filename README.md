...
for `CLAUDE_CODE_OAUTH_TOKEN`, `mcp.json`, and `CLAUDE.md`.
...
- **Identity:** 1 Pod = 1 Anthropic identity. Authenticate via `CLAUDE_CODE_OAUTH_TOKEN` (long-lived 1-year token from `claude setup-token`) — see [Identity model](docs/operations.md#identity-model-1x).
...
Handoff.md                # design history & decision log
```
...
전체 k8s manifest 예시는 `deploy/k8s/` 참고. k8s 1.28+ native 사이드카 (`initContainer` + `restartPolicy: Always`) 패턴, `CLAUDE_CODE_OAUTH_TOKEN` / `mcp.json` / `CLAUDE.md`의 Secret · ConfigMap 레이아웃을 포함합니다.
...
- **인증**: 1 Pod = 1 Anthropic 신원. `claude setup-token`으로 1년짜리 long-lived 토큰을 받아 `CLAUDE_CODE_OAUTH_TOKEN` 환경변수로 주입 — 자세한 절차는 [Identity model](docs/operations.md#identity-model-1x)
...
Handoff.md                # 설계 히스토리 · 결정 기록
```

## 개발

```bash
.venv/bin/python -m pytest          # 42 테스트
.venv/bin/ruff check .              # lint
